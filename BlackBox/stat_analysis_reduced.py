...
def perimp_analysis(filename, per_means, per_sems):
    ...
    param_names = ['ant_Kd', 'Kcat_LCKPU_CD3z', 'CSKon', 'kcat_ZAP', 'Kcat_CD45_LCK505', 'Kcat_CD45_A1']

    mplt.figure()
    mplt.bar(np.arange(1,7), per_means[[(0,7,10,19,42,43)]], yerr=per_sems[[(0,7,10,19,42,43)]], capsize=5)
    #mplt.title('Permutation Importances for Parameters, Data Set: '+filename, fontdict={'fontsize':25})
    mplt.xticks(ticks=np.arange(1,7), labels=param_names, fontsize=50, rotation=45, fontweight='bold')
    
    mplt.ylim(0,0.7)
    mplt.yticks(ticks=np.arange(0,0.7,0.1), fontsize=50, fontweight='bold') 
    
    mplt.xlabel("Parameter Name", fontsize=75, fontweight='bold')
    mplt.ylabel("Importance Score", fontsize=75, fontweight='bold')
    
    #mplt.plot(np.arange(1,7), (per_means[[(0,7,10,19,42,43)]]+0.02)*(p_val[[(0,7,10,19,42,43)]]<0.01)-0.001, marker='*', ms=15, color='k', linestyle="None")
    #mplt.plot(np.arange(1,7), (per_means[[(0,7,10,19,42,43)]]+0.03)*(p_val[[(0,7,10,19,42,43)]]<0.01)-0.001, marker='*', ms=15, color='k', linestyle="None")
    return p_val
...
def perimp_analysis_o(filename, per_means, per_sems):
    ...
    mplt.figure()
    mplt.bar(np.arange(1,7), per_means[[(0,7,10,19,42,43)]], yerr=per_sems[[(0,7,10,19,42,43)]], capsize=5, color=[1,0.4,0,1])
    #mplt.title('Permutation Importances for Parameters, Data Set: '+filename, fontdict={'fontsize':25})
    mplt.xticks(ticks=np.arange(1,7), labels=param_names, fontsize=50, rotation=45, fontweight='bold')
